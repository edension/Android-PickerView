...
public class WheelView extends View {
    ...
    // 内外的字体样式 added by lyh 19.9.27
    private Typeface outTypeface = Typeface.MONOSPACE;//字体样式，默认是等宽字体
    private Typeface centerTypeface = Typeface.MONOSPACE;//字体样式，默认是等宽字体
    ...
    private void initPaints() {
        ...
        paintOuterText.setTextSize(getTextSizeOut());
    ...
    /**
     * 设置内外的字体样式
     * added by lyh 19.9.27
     * @param centerTypeface
     * @param outTypeface
     */
    public final void setTypeface(Typeface centerTypeface, Typeface outTypeface) {
        this.outTypeface = outTypeface;
        this.centerTypeface = centerTypeface;
        paintOuterText.setTypeface(outTypeface);
        paintCenterText.setTypeface(centerTypeface);
    }
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    // 控制透明度
                    // 字体颜色的透明度作为上限 added by lyh 19.9.27
                    paintOuterText.setAlpha((int) ((1 - offsetCoefficient) * /*255*/ Color.alpha(textColorOut)));
                    // 控制文字水平偏移距离
    ...
    /**
     * 根据textSize，初始化外部文字的大小
     * added by lyh 19.9.27
     */
    private void initTextSizeOut() {
        int offsetSize = (int) (getResources().getDisplayMetrics().scaledDensity * 8 + 0.5f);
        textSizeOut = textSize - offsetSize;
    }

    // added by lyh 19.9.27
    private int getTextSizeOut() {
        return /*textSize*/ textSizeOut;
    }
}